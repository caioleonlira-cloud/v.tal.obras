...
export const DashboardView: React.FC<DashboardViewProps> = ({ onNavigateToRegistros }) => {
  ...
  const {
    ...
    registros.forEach((r) => {
      ...
      if (matchesFilterSubset(r, 'UF')) uCounts[ufVal] = (uCounts[ufVal] || 0) + 1;
      else if (filterUF.includes(ufVal) && !uCounts[ufVal]) uCounts[ufVal] = 0;
      ...
      // 4. Tipo DC
      const tdcVal = getRegistroFilterValue(r, 'TIPO_DC');
      if (tdcVal === BLANK_FILTER_OPTION) hasBlankTdc = true;
      ...
      if (matchesFilterSubset(r, 'AGING')) agCounts[agVal] = (agCounts[agVal] || 0) + 1;
      else if (filterAging.includes(agVal) && !agCounts[agVal]) agCounts[agVal] = 0;

      // 6. Status Atual
      const stAtualVal = getRegistroFilterValue(r, 'STATUS_ATUAL');
      if (stAtualVal === BLANK_FILTER_OPTION) hasBlankStAtual = true;
      if (matchesFilterSubset(r, 'STATUS_ATUAL')) stAtualCounts[stAtualVal] = (stAtualCounts[stAtualVal] || 0) + 1;
      else if (filterStatusAtual.includes(stAtualVal) && !stAtualCounts[stAtualVal]) stAtualCounts[stAtualVal] = 0;

      // 7. Status Informe
      const stInfVal = getRegistroFilterValue(r, 'STATUS_INFORME');
      if (stInfVal === BLANK_FILTER_OPTION) hasBlankStInf = true;
      if (matchesFilterSubset(r, 'STATUS_INFORME')) stInfCounts[stInfVal] = (stInfCounts[stInfVal] || 0) + 1;
      else if (filterStatusInforme.includes(stInfVal) && !stInfCounts[stInfVal]) stInfCounts[stInfVal] = 0;
      ...
      if (matchesFilterSubset(r, 'TIPO_PROJETO')) projCounts[projVal] = (projCounts[projVal] || 0) + 1;
      else if (filterTipoProjeto.includes(projVal) && !projCounts[projVal]) projCounts[projVal] = 0;
      ...
      if (matchesFilterSubset(r, 'STATUS_MED_PARCIAL')) stMedParcCounts[stMedParcVal] = (stMedParcCounts[stMedParcVal] || 0) + 1;
      else if (filterStatusMedParcial.includes(stMedParcVal) && !stMedParcCounts[stMedParcVal]) stMedParcCounts[stMedParcVal] = 0;

      // 11. Status Med Final
      const stMedFinVal = getRegistroFilterValue(r, 'STATUS_MED_FINAL');
      if (stMedFinVal === BLANK_FILTER_OPTION) hasBlankStMedFin = true;
      if (matchesFilterSubset(r, 'STATUS_MED_FINAL')) stMedFinCounts[stMedFinVal] = (stMedFinCounts[stMedFinVal] || 0) + 1;
      else if (filterStatusMedFinal.includes(stMedFinVal) && !stMedFinCounts[stMedFinVal]) stMedFinCounts[stMedFinVal] = 0;

      // 12. Backlog / Input
      const peVal = getRegistroFilterValue(r, 'BACKLOG_INPUT');
      if (peVal === BLANK_FILTER_OPTION) hasBlankBacklog = true;
      ...
      // 13. Mês Input
      const miVal = getRegistroFilterValue(r, 'MES_INPUT');
      if (miVal === BLANK_FILTER_OPTION) hasBlankMesInp = true;
      ...
      // 14. Resp. Medição
      const rmVal = getRegistroFilterValue(r, 'RESP_MEDICAO');
      if (rmVal === BLANK_FILTER_OPTION) hasBlankRespMed = true;
    ...
    return {
      ...
      tipoDCOptions: sortFilterOptions(Object.keys(tdcCounts)),
